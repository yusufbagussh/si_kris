...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create('edc_payments', function (Blueprint $table) {
            ...
            //$table->enum('method', ['purchase', 'brizzi', 'qris'])->default('purchase')->comment('Metode pembayaran');
            //$table->enum('status', ['success', 'failed', 'unpaid', 'paid', 'refund'])->default('pending')->comment('Status transaksi');
            //$table->enum('action', ['Sale', 'Contactless', 'Void', 'Check Status', 'Refund Qris', 'Reprint Last', 'Reprint Any', 'Settlement"'])->nullable()->comment('Nama Service EcrLink');
            $table->string('method', 50)->comment('Metode pembayaran');
            $table->string('status', 50)->comment('Status transaksi');
            $table->string('action', 50)->comment('Nama Service EcrLink');
            ...
            $table->string('is_credit')->nullable()->comment('Apakah kartu kredit atau debit');
            $table->string('is_off_us')->nullable()->comment('Apakah kartu non-BRI');
